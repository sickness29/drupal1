...
function my_theme_preprocess_html(&$vars) {
  $vars['head_title'] .= ' - |preprocess_html()|';
...
function my_theme_preprocess_node(&$vars) {
  if($vars['view_mode']=='full') {
    drupal_set_message('Node id = ' . $vars['nid'] . ' - preprocess_node()');
  }
}
...
function my_theme_preprocess_field(&$vars) {
  ...
  if ($vars['element']['#field_name'] == 'field_picture') {
    drupal_set_message($vars['items'][0]['#item']['filename'] . ' - preprocess_field()');
  }
}
...
function my_theme_preprocess_block(&$vars) {
  if ($vars['elements']['#block']->delta == 'some-block') {
    drupal_set_message($vars['elements']['#children'] . ' - preprocess_block()');
  }
}
...
function my_theme_preprocess_views_view(&$vars) {
  if ($vars['view']->name == 'first_block') {
    $vars['title'] = '<h5> preprocess_views_view() </h5>';
    $vars['rows'] = '<strong>' . $vars['rows'] . '</strong>';
  }
}
...
function my_theme_preprocess_page(&$vars, $hook) {
  if (isset($vars['node']) && $vars['node']->nid == 223) {
